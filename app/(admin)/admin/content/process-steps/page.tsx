...
interface ProcessStep { id: string; title: string; description: string; order: number }
...
export default function ProcessStepsPage() {
  ...
  async function load() { const res = await fetch('/api/admin/process-steps'); setItems(await res.json()); setLoading(false) }
  useEffect(() => { load() }, [])

  function openCreate() { setEditing(null); reset({ title: '', description: '' }); setShowForm(true) }
  function openEdit(s: ProcessStep) { setEditing(s); reset({ title: s.title, description: s.description }); setShowForm(true) }
  ...
  async function confirmDelete() {
    ...
    setDeleting(true)
    await fetch(`/api/admin/process-steps/${deleteTarget.id}`, { method: 'DELETE' })
  ...
  return (
    <div className="max-w-3xl">
      <AdminPageHeader
        ...
        description="Manage the numbered steps shown on the Support page ('How to take the first step')."
        action={
          <div className="flex items-center gap-2">
            <a href="/support" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 h-10 px-3 rounded-lg border border-border text-sm text-text-muted hover:text-text hover:bg-gray-50 transition-colors">
      ...
      {showForm && (
        <AdminCard title={editing ? 'Edit step' : 'Add step'} className="mb-6">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <AdminFormField label="Step title" name="title" required register={register('title', { required: 'Required' })} error={errors.title?.message} placeholder="Reach Out" />
      ...
      <AdminCard title="Process Steps" description={`${items.length} step${items.length !== 1 ? 's' : ''} — displayed in order`}>
        {loading ? <AdminListSkeleton rows={4} /> : items.length === 0 ? <AdminEmptyState title="No steps" /> : (
          <ol className="divide-y divide-border/50">
            {items.map((s, idx) => (
              <li key={s.id} className="flex items-start gap-4 py-3">
                <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-sm shrink-0">{idx + 1}</div>
